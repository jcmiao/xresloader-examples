...
namespace excel {
    ...
    int config_set_role_upgrade_cfg::on_inited() {
    ...
    int config_set_role_upgrade_cfg::load_all() {
        int ret = 0;
        ::excel::lock::write_lock_holder<::excel::lock::spin_rw_lock> wlh(load_file_lock_);
        for (std::unordered_map<std::string, bool>::iterator iter = file_status_.begin(); iter != file_status_.end(); ++ iter) {
            if (!iter->second) {
                ...
                if (res < 0) {
                    EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load config file %s for %s failed", iter->first.c_str(), "config_set_role_upgrade_cfg");
                    ret = res;
                } else if (ret >= 0) {
                    ret += res;
                }
    ...
    void config_set_role_upgrade_cfg::clear() {
        ::excel::lock::write_lock_holder<::excel::lock::spin_rw_lock> wlh(load_file_lock_);
        id_data_.clear();
        id_level_data_.clear();
        file_status_.clear();
        datasource_.clear();
        reload_file_lists();
    }

    const std::list<org::xresloader::pb::xresloader_data_source>& config_set_role_upgrade_cfg::get_data_source() const {
        return datasource_;
    }

    int config_set_role_upgrade_cfg::load_file(const std::string& file_path) {
        std::unordered_map<std::string, bool>::iterator iter = file_status_.find(file_path);
        if (iter == file_status_.end()) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load config file %s for %s failed, not exist in any file_list/file_path", file_path.c_str(), "config_set_role_upgrade_cfg");
            return -2;
        }
        ...
        if (!config_manager::me()->load_file_data(content, file_path)) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load file %s for %s failed", file_path.c_str(), "config_set_role_upgrade_cfg");
            return -3;
        }
        ...
        if (!outer_data.ParseFromString(content)) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] parse file %s for %s(message type: %s) failed: %s",
                file_path.c_str(), "config_set_role_upgrade_cfg", "::org::xresloader::pb::xresloader_datablocks",
        ...
        for (int i = 0; i < outer_data.data_block_size(); ++ i) {
            ...
            if (!new_item) {
                EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] parse file %s for %s(message type: %s) and create item object %d failed",
                    file_path.c_str(), "config_set_role_upgrade_cfg", "::org::xresloader::pb::xresloader_datablocks", i
                );
                return -5;
            }
            ...
            if (!std::const_pointer_cast<proto_type>(new_item)->ParseFromString(outer_data.data_block(i))) {
                EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] parse message %d in %s for %s(message type: %s) failed: %s",
                    i, file_path.c_str(), "config_set_role_upgrade_cfg", "::org::xresloader::pb::xresloader_datablocks",
    ...
    int config_set_role_upgrade_cfg::load_list(const char* file_list_path) {
        std::string content;
        if (!config_manager::me()->load_file_data(content, file_list_path)) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load file %s for %s failed", file_list_path, "config_set_role_upgrade_cfg");
            return -1;
        }
    ...
    int config_set_role_upgrade_cfg::reload_file_lists() {
        file_status_["../config/sample-data/role_upgrade_cfg.bin"] = false;
        return 0;
    }

    void config_set_role_upgrade_cfg::merge_data(item_ptr_type item) {
        if (!item) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] merge_data(nullptr) is not allowed for %s", "config_set_role_upgrade_cfg");
            return;
        }

        // index: id
        do {
            id_data_[std::make_tuple(item->id())].push_back(item);
        } while(false);

        // index: id_level
        do {
            std::tuple<uint32_t, uint32_t> key = std::make_tuple(item->id(), item->level());
            if (id_level_data_.end() != id_level_data_.find(key)) {
                EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] merge_data() with key=<%u, %u> for %s is already exists, we will cover it with the newer value", 
                    static_cast<unsigned int>(item->id()), static_cast<unsigned int>(item->level()), "config_set_role_upgrade_cfg");
            }
            id_level_data_[key] = item;
        } while(false);

    }

// ------------------- index: id APIs -------------------
    const config_set_role_upgrade_cfg::id_value_type* config_set_role_upgrade_cfg::get_list_by_id(uint32_t Id) {
        ::excel::lock::read_lock_holder<::excel::lock::spin_rw_lock> rlh(load_file_lock_);
        return _get_list_by_id(Id);
    }

    config_set_role_upgrade_cfg::item_ptr_type config_set_role_upgrade_cfg::get_by_id(uint32_t Id, size_t index) {
        ::excel::lock::read_lock_holder<::excel::lock::spin_rw_lock> rlh(load_file_lock_);
        const config_set_role_upgrade_cfg::id_value_type* list_item = _get_list_by_id(Id);
        if (nullptr == list_item) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load index %s with key=<%u>, index=%llu for %s failed, list not found",
                ...
                static_cast<unsigned long long>(index), "config_set_role_upgrade_cfg"
            );
            if (config_manager::me()->get_on_not_found()) {
                config_manager::on_not_found_event_data_t evt_data;
                evt_data.data_source = &datasource_;
                evt_data.message_descriptor = ::role_upgrade_cfg::descriptor();
        ...
        if (list_item->size() <= index) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load index %s with key=<%u>, index=%llu for %s failed, index entend %llu",
                "id", static_cast<unsigned int>(Id), 
                static_cast<unsigned long long>(index), "config_set_role_upgrade_cfg", static_cast<unsigned long long>(list_item->size())
            );
            if (config_manager::me()->get_on_not_found()) {
                config_manager::on_not_found_event_data_t evt_data;
                evt_data.data_source = &datasource_;
    ...
    const config_set_role_upgrade_cfg::id_value_type* config_set_role_upgrade_cfg::_get_list_by_id(uint32_t Id) {
        id_container_type::iterator iter = id_data_.find(std::make_tuple(Id));
        if (iter != id_data_.end()) {
            return &iter->second;
        }

        std::string file_path = "../config/sample-data/role_upgrade_cfg.bin";

        int res = load_file(file_path);
        if (res < 0) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load file %s for %s failed, res: %d", file_path.c_str(), "config_set_role_upgrade_cfg", res);
            return nullptr;
        }
        ...
        if (iter == id_data_.end()) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load index %s with key=<%u> for %s failed, not found",
                "id", static_cast<unsigned int>(Id), "config_set_role_upgrade_cfg"
            );
            if (config_manager::me()->get_on_not_found()) {
                config_manager::on_not_found_event_data_t evt_data;
                evt_data.data_source = &datasource_;
    ...
    const config_set_role_upgrade_cfg::id_container_type& config_set_role_upgrade_cfg::get_all_of_id() const {
        return id_data_;
    }
// ------------------- index: id_level APIs -------------------
    config_set_role_upgrade_cfg::id_level_value_type config_set_role_upgrade_cfg::get_by_id_level(uint32_t Id, uint32_t Level) {
        ::excel::lock::read_lock_holder<::excel::lock::spin_rw_lock> rlh(load_file_lock_);
        id_level_container_type::iterator iter = id_level_data_.find(std::make_tuple(Id, Level));
        if (iter != id_level_data_.end()) {
            return iter->second;
        }
        ...
        iter = id_level_data_.find(std::make_tuple(Id, Level));
        if (iter == id_level_data_.end()) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load index %s with key=<%u, %u> for %s failed, not found",
                "id_level", static_cast<unsigned int>(Id), static_cast<unsigned int>(Level), "config_set_role_upgrade_cfg"
            );
            if (config_manager::me()->get_on_not_found()) {
                config_manager::on_not_found_event_data_t evt_data;
                evt_data.data_source = &datasource_;
                ...
                char keys_buffer[4096];
                int n = UTIL_STRFUNC_SNPRINTF(keys_buffer, sizeof(keys_buffer) - 1, "%u, %u", static_cast<unsigned int>(Id), static_cast<unsigned int>(Level));
                evt_data.index_name = "id_level";
                if (n < static_cast<int>(sizeof(keys_buffer)) && n >= 0) {
                    keys_buffer[n] = 0;
                } else {
                    keys_buffer[sizeof(keys_buffer) - 1] = 0;
    ...
    const config_set_role_upgrade_cfg::id_level_container_type& config_set_role_upgrade_cfg::get_all_of_id_level() const {
        return id_level_data_;
    }
