...
namespace excel {
    ...
    config_set_quests_cfg::config_set_quests_cfg() {
    }

    config_set_quests_cfg::~config_set_quests_cfg(){
    }

    int config_set_quests_cfg::on_inited() {
    ...
    int config_set_quests_cfg::load_all() {
        int ret = 0;
        ::excel::lock::write_lock_holder<::excel::lock::spin_rw_lock> wlh(load_file_lock_);
        for (std::unordered_map<std::string, bool>::iterator iter = file_status_.begin(); iter != file_status_.end(); ++ iter) {
            if (!iter->second) {
                ...
                if (res < 0) {
                    EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load config file %s for %s failed", iter->first.c_str(), "config_set_quests_cfg");
                    ret = res;
                } else if (ret >= 0) {
                    ret += res;
                }
    ...
    void config_set_quests_cfg::clear() {
        ::excel::lock::write_lock_holder<::excel::lock::spin_rw_lock> wlh(load_file_lock_);
        id_data_.clear();
        type_data_.clear();
        file_status_.clear();
    ...
    const std::list<org::xresloader::pb::xresloader_data_source>& config_set_quests_cfg::get_data_source() const {
        return datasource_;
    }

    int config_set_quests_cfg::load_file(const std::string& file_path) {
        std::unordered_map<std::string, bool>::iterator iter = file_status_.find(file_path);
        if (iter == file_status_.end()) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load config file %s for %s failed, not exist in any file_list/file_path", file_path.c_str(), "config_set_quests_cfg");
            return -2;
        }
        ...
        if (!config_manager::me()->load_file_data(content, file_path)) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load file %s for %s failed", file_path.c_str(), "config_set_quests_cfg");
            return -3;
        }
        ...
        if (!outer_data.ParseFromString(content)) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] parse file %s for %s(message type: %s) failed: %s",
                file_path.c_str(), "config_set_quests_cfg", "::org::xresloader::pb::xresloader_datablocks",
        ...
        for (int i = 0; i < outer_data.data_block_size(); ++ i) {
            ...
            if (!new_item) {
                EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] parse file %s for %s(message type: %s) and create item object %d failed",
                    file_path.c_str(), "config_set_quests_cfg", "::org::xresloader::pb::xresloader_datablocks", i
                );
                return -5;
            }
            ...
            if (!std::const_pointer_cast<proto_type>(new_item)->ParseFromString(outer_data.data_block(i))) {
                EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] parse message %d in %s for %s(message type: %s) failed: %s",
                    i, file_path.c_str(), "config_set_quests_cfg", "::org::xresloader::pb::xresloader_datablocks",
    ...
    int config_set_quests_cfg::load_list(const char* file_list_path) {
        std::string content;
        if (!config_manager::me()->load_file_data(content, file_list_path)) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load file %s for %s failed", file_list_path, "config_set_quests_cfg");
            return -1;
        }
    ...
    int config_set_quests_cfg::reload_file_lists() {
        file_status_["../config/sample-data/quests_cfg.bin"] = false;
        return 0;
    }

    void config_set_quests_cfg::merge_data(item_ptr_type item) {
        if (!item) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] merge_data(nullptr) is not allowed for %s", "config_set_quests_cfg");
            return;
        }
        ...
        do {
            ...
            if (id_data_.end() != id_data_.find(key)) {
                EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] merge_data() with key=<%u> for %s is already exists, we will cover it with the newer value", 
                    static_cast<unsigned int>(item->id()), "config_set_quests_cfg");
            }
            id_data_[key] = item;
        } while(false);
...
// ------------------- index: id APIs -------------------
    config_set_quests_cfg::id_value_type config_set_quests_cfg::get_by_id(uint32_t Id) {
        ...
        std::string file_path = "../config/sample-data/quests_cfg.bin";

        int res = load_file(file_path);
        if (res < 0) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load file %s for %s failed, res: %d", file_path.c_str(), "config_set_quests_cfg", res);
            return nullptr;
        }
    ...
    const config_set_quests_cfg::id_container_type& config_set_quests_cfg::get_all_of_id() const {
        return id_data_;
    }
// ------------------- index: type APIs -------------------
    const config_set_quests_cfg::type_value_type* config_set_quests_cfg::get_list_by_type(uint32_t Type) {
        ::excel::lock::read_lock_holder<::excel::lock::spin_rw_lock> rlh(load_file_lock_);
        return _get_list_by_type(Type);
    }

    config_set_quests_cfg::item_ptr_type config_set_quests_cfg::get_by_type(uint32_t Type, size_t index) {
        ::excel::lock::read_lock_holder<::excel::lock::spin_rw_lock> rlh(load_file_lock_);
        const config_set_quests_cfg::type_value_type* list_item = _get_list_by_type(Type);
        if (nullptr == list_item) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load index %s with key=<%u>, index=%llu for %s failed, list not found",
                ...
                static_cast<unsigned long long>(index), "config_set_quests_cfg"
            );
            if (config_manager::me()->get_on_not_found()) {
                config_manager::on_not_found_event_data_t evt_data;
                evt_data.data_source = &datasource_;
        ...
        if (list_item->size() <= index) {
            EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load index %s with key=<%u>, index=%llu for %s failed, index entend %llu",
                "type", static_cast<unsigned int>(Type), 
                static_cast<unsigned long long>(index), "config_set_quests_cfg", static_cast<unsigned long long>(list_item->size())
            );
            if (config_manager::me()->get_on_not_found()) {
                config_manager::on_not_found_event_data_t evt_data;
                evt_data.data_source = &datasource_;
    ...
    const config_set_quests_cfg::type_value_type* config_set_quests_cfg::_get_list_by_type(uint32_t Type) {
        size_t idx = 0;
        ...
        EXCEL_CONFIG_MANAGER_LOGERROR("[EXCEL] load index %s with key=<%u> for %s failed, not found",
            "type", static_cast<unsigned int>(Type), "config_set_quests_cfg"
        );
        if (config_manager::me()->get_on_not_found()) {
            config_manager::on_not_found_event_data_t evt_data;
            evt_data.data_source = &datasource_;
    ...
    const config_set_quests_cfg::type_container_type& config_set_quests_cfg::get_all_of_type() const {
        return type_data_;
    }
